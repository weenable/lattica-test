...
def main():
    ...
    if bootstrap:
        # init
        lattica = Lattica.builder().with_bootstraps([bootstrap]).with_listen_addrs(["/ip4/0.0.0.0/tcp/19090"]).with_mdns(False).build()
        ...
        # init
        lattica = Lattica.builder().with_mdns(False).with_listen_addrs(["/ip4/0.0.0.0/tcp/19090"]).build()
